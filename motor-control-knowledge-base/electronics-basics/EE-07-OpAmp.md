...
差模增益：G_diff = R2/R1（假设 R1=R3, R2=R4）
CMRR依赖 R2/R1 = R4/R3 的匹配精度！

1%电阻匹配 → CMRR_max = (1+R2/R1)/(4×ε)
  R2/R1=1, ε=1% → CMRR_max = 2/(4×0.01) = 50 ≈ 34dB  ← 惨不忍睹！
0.1%电阻匹配 → CMRR_max = 2/(4×0.001) = 500 ≈ 54dB
0.01%电阻网络 → CMRR_max = 2/(4×0.0001) = 5000 ≈ 74dB

结论：四电阻差分放大器要想获得高CMRR，必须用精密匹配电阻网络！
```
...
```
电流采样链：
  Shunt(10mΩ) → 差分/仪放(G=50) → LPF(fc=2kHz) → ADC(12bit, 3.3V)

ADC分辨率：3.3V/4096 = 0.806mV/LSB
折算到shunt电压：0.806mV/50 = 16.1μV → 电流分辨率 = 16.1μV/10mΩ = 1.61mA

运放Vos=2mV(LM358)的影响：
  输出误差 = 2mV×50 = 100mV → 124 LSB → 电流误差 = 200mA ← 不可接受!

运放Vos=10μV(OPA333)的影响：
  输出误差 = 10μV×50 = 0.5mV → <1 LSB → 可忽略 ✅
...
```
DSP软件过流保护的延迟：
  ADC采样(1μs) + 中断响应(2μs) + 判断(0.5μs) + PWM关断(1μs) = 4.5μs
  
IGBT短路耐受：6μs
  6μs > 4.5μs → 理论上可行，但裕量仅1.5μs! 风险极高!

硬件比较器保护：
  运放做比较器(tpd=50ns) + 逻辑门(20ns) + PWM关断(100ns) = 170ns
  6μs >> 170ns → 裕量充足 ✅✅✅

推荐架构：
  运放比较器 → PWM关断（硬件，<200ns）← 第一道防线
  ADC → DSP判断 → PWM关断（软件，~5μs）← 第二道防线
```
