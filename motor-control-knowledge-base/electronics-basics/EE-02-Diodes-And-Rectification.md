...
```
P型半导体（多"空穴"+）          N型半导体（多"电子"-）
  +  +  +  +  +  +       |      -  -  -  -  -  -
  +  +  +  +  +  +       |      -  -  -  -  -  -
  +  +  +  +  +  +       |      -  -  -  -  -  -
  ←── 耗尽层（无载流子区）──→
  
正向偏置：P接正，N接负 → 耗尽层变窄 → 电子和空穴跨越PN结 → 导通
反向偏置：P接负，N接正 → 耗尽层变宽 → 载流子无法跨越 → 截止
```
...
输出电压：Vdc_avg = Vpeak/π ≈ 0.318 × Vpeak
纹波频率 = 输入频率（50/60Hz）
```
...
三相桥式整流（电机驱动常用）：
  ...
  输出：Vdc = 1.35 × Vline_rms（三相）
       Vdc = 1.414 × Vphase_rms（单相）
```
...
充电阶段（Q_L导通，HS拉到GND）：
  VCC → D_boot → Cboot → GND
  V_boot = VCC - Vf(D_boot) ≈ 12V - 0.4V = 11.6V（肖特基）
  
放电阶段（Q_L关断，Q_H导通，HS拉到Vdc）：
  V_boot悬浮到 Vdc + (VCC - Vf)
  → 高边栅极获得高于Vdc的驱动电压！
```
...
```
电网AC → 三相整流桥 → 直流母线 → 逆变器 → 电机

关键计算：
  Vdc ≈ 1.35 × Vline_rms（三相全桥，空载）
  Vdc ≈ 1.35 × 380V = 513V（带电容滤波后可达√2×380=537V!）
```
