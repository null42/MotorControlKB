...
电网供电是"脉冲式的"，每秒注入300次能量脉冲（6脉波整流）
电容像水库，在脉冲间隙向逆变器放水，平滑母线电压
```
...
输出电压：
  Vdc_avg = 0.9 × Vac（空载）
  Vdc_peak = √2 × Vac ≈ 1.414 × Vac

  220V单相：Vdc ≈ 1.414×220 ≈ 311V
...
输出电压：
  Vdc_avg = 1.35 × VLL（理想空载）
  Vdc_peak = √2 × VLL ≈ 1.414 × VLL

  三相380V：Vdc ≈ 1.35×380 = 513V
  带载后由于内阻和滤波电容，实际≈500~540V

纹波频率：300Hz（6倍电网频率50Hz）→纹波远小于单相
```
...
上电时NTC冷态电阻≈10Ω → I_inrush=540/10=54A（可控）
运行后NTC自发热→热态电阻≈0.2Ω →仅损耗 I²×0.2W
...
时序：
1. 继电器初始断开
2. 上电→经R_charge对电容充电（τ = R_charge × C）
3. 电容电压达到稳定值的80%~90%（约3~5τ）
4. MCU检测母线电压→闭合继电器→旁路R_charge

参数计算（20000μF, 540V）：
  R_charge = 47Ω/50W
  τ = 47Ω × 20000μF = 0.94s
  5τ = 4.7s（充满到99.3%）
  I_inrush = 540V/47Ω = 11.5A ✅（远小于整流桥的IFSM）

  充电电阻功率：
  最差情况：电压均方根
  P_avg ≈ (Vdc²/R) × (τ_charge/T)  
  实际选择50W铝壳电阻（短时工作，3~5秒不损坏）
```
...
6个开关管：Q1~Q6（IGBT或MOSFET）
每个开关管并联一个续流二极管（通常集成在IGBT内部）
```
...
参考电压矢量 Vref 在 αβ 平面内旋转
- |Vref_max| = 2Vdc/3 × cos(30°) = Vdc/√3 ≈ 0.577Vdc（内切圆）
```
...
```
SPWM = 正弦波(Vref_sine) 与 三角载波(Vcarrier) 比较
...
调制比 ma = V_ref_peak / V_carrier_peak（0 ≤ ma ≤ 1）
```
...
```
Vdc+ ────→ R_brake ───→ Q_brake ───→ Vdc-
                │
            制动电阻    制动IGBT

Vdc > V_threshold → 开通Q_brake → 母线上多余能量通过R_brake消耗

R_brake计算：
  R_brake = Vdc_th² / P_brake_peak
  
  P_brake_peak = E_brake × (1/t_decel) （平均制动功率）
```
