...
NPN：基极电流流入(Ib>0) → 集电极电流从C流向E
PNP：基极电流流出(Ib<0) → 集电极电流从E流向C
  
图腾柱驱动 = NPN + PNP 上下组合：
  NPN在上面：拉电流(source) → 对栅极电容充电
  PNP在下面：灌电流(sink)   → 对栅极电容放电
```
...
工作原理：
  1. B-E正向偏置(Vbe>0.7V) → 电子从E注入B（发射）
  2. 基区极薄 → 大部分电子来不及复合就到达C（传输）
  3. C-B反向偏置 → 电子被C收集（收集）
...
输入高 → Q1导通,Q2截止 → OUT拉高(Source) → 栅极充电
输入低 → Q1截止,Q2导通 → OUT拉低(Sink)   → 栅极放电
```
...
```
MCU PWM → 图腾柱BJT → MOSFET栅极

传统离散方案：
  2N2222(NPN) + 2N2907(PNP) → 廉价但速度慢
  2SC4672 + 2SA1797 → 高频低饱和

集成方案：
  栅极驱动IC内置图腾柱 → IR2110, IRS2186等
  峰值电流可达2~4A
```
