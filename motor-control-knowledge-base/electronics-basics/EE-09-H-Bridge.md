...
四种运行模式：
  Q1+Q4 导通 → 电流从左到右 → 电机正转
  Q2+Q3 导通 → 电流从右到左 → 电机反转
  Q1+Q3 或 Q2+Q4 导通 → 电机两端短路 → 制动（能耗制动）
  全部关断 → 电机惯性旋转 → 发电（反电动势回馈）
```
...
两个开关管串联在Vdc和GND之间
输出节点在中间，可接到Vdc（Q_H导通）或GND（Q_L导通）

用于：三相逆变器的一个桥臂（3个半桥=三相逆变器）
```
...
充电阶段（下管导通）：
  Vcc → D_bs → C_boot → Q_L → GND
  C_boot电压 = Vcc - Vf(D_bs) ≈ Vcc - 0.7V

升压阶段（下管关断，上管准备导通）：
  下管关断→自举电容负端（S_H）被上拉
  S_H ≈ Vdc → V_boot = Vdc + Vcc（自举！）
  栅极驱动用Vboot驱动上管：V_GS = (Vdc+Vcc) - Vdc ≈ Vcc ✅
```
...
上下管同时导通 → Vdc到GND近似短路
短路电流仅受MOSFET导通电阻限制（数十mΩ）→数百A电流 →炸管！
```
...
优点：共模电压为0，运放设计简单，成本低
缺点：只能在下管导通时采样→需要同步采样策略
      不能直接采样续流电流
      三电阻方案成本高于单电阻
```

```
方案2：相线采样（Phase/In-Line Sensing）
        各相输出线串shunt电阻
        ┌─Q_H─┬─Q_H─┬─Q_H─┐
        │     │     │     │
        ├──R──┼──R──┼──R──┤── 电机
...
优点：可全程连续采样相电流（包括续流阶段）
     电流信号连续→适合FOC控制
缺点：共模电压在0~Vdc之间跳变→需要高CMRR运放
     三电阻方案
```

```
方案3：母线采样（Bus Sensing）
        在母线负端与GND之间串一个shunt
             Vdc
              │
        ┌─────┼─────┐
...
优点：只需一个shunt→成本最低
缺点：读到的不是相电流，是母线电流（斩波后）→需要复杂重构算法
     不能用于FOC（需要连续相电流）
     仅适用于BLDC方波驱动（不需要连续电流信息）
```
...
```
H桥的再生制动（Regenerative Braking）：

电机减速时，反电动势使相电压高于母线电压：
  → 续流二极管正向偏置
  → 电流从电机回灌到母线电容
  → Vdc上升
