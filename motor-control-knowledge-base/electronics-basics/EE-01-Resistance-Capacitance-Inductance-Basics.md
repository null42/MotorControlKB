...
**与电机控制的关联**：
- 🔗 **电机绕组(Rs + Ld/Lq)**：决定电流环PI参数 → `Kp = L × ωc, Ki = R × ωc`
- 🔗 **直流母线电容(Cdc)**：决定母线电压纹波 → `ΔV = Iload / (2 × f_PWM × Cdc)`
- 🔗 **采样电阻(Rshunt)**：决定电流测量精度 → `Vshunt = Iphase × Rshunt`
- 🔗 **LC谐振(ω = 1/√(LC))**：EMI滤波器设计、变压器设计
...
✅ **掌握RC时间常数** - `τ = RC`，滤波截止频率 `fc = 1/(2πRC)`
✅ **掌握LR时间常数** - `τ = L/R`，电机绕组电流上升速率
✅ **掌握LC谐振** - `ω = 1/√(LC)`，EMI滤波器自谐振频率
...
**电机中的应用**：
- 定子绕组电阻Rs → 铜损 `Pcu = 3 × I² × Rs`
- 电流采样电阻 → `Vshunt = Iphase × Rshunt`（0.1~10mΩ典型值）
...
示例：`Le = 5nH, di/dt = 100A/μs → Vspike = 5nH × 100A/μs = 0.5V`
...
电解电容的等效模型：
```
  o────ESR───L(ESL)───C────o  真实电容模型
  
  电解电容典型参数：
    100μF/450V：ESR ≈ 0.5~2Ω, ESL ≈ 10~50nH
  
  陶瓷电容典型参数：
    10μF/50V MLCC：ESR ≈ 2~10mΩ, ESL ≈ 0.5~2nH
```
...
```
采样电阻Rshunt的选择：
  [电流→电压]  Vshunt = Iphase × Rshunt
  [电压→数字]  ADC_code = Vshunt × 4096 / 3.3 (12位ADC)
...
**经验法则**：采样电阻选型四步——(1) 最大电流×Rshunt 不超过运放输入范围；(2) 最小电流×Rshunt > ADC噪声；(3) `I²R`损耗 < 0.5W（避免自热误差）；(4) 选用4端子Kelvin连接电阻，消除引线电阻影响。
...
硬件参数：Ld, Lq, Rs (电机绕组)
    ↓ 零极点对消设计
控制参数：Kp = L × ωc, Ki = R × ωc

如果L测量偏差30% → Kp偏差30% → 电流环带宽偏差30%
→ 震荡或响应迟钝！
```
...
方案C:增加RC滤波 ✅
  R = 10Ω, C = 1nF, fc = 1/(2π×10×1e-9) = 15.9MHz
  > 注意:fc不能太低,否则电流环延迟增大!
```
