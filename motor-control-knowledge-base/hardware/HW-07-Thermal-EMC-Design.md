...
热阻网络：
  结温(Tj) ──Rth_JC──→ 外壳温度(Tc) ──Rth_CS──→ 散热片温度(Ts) ──Rth_SA──→ 环境温度(Ta)
...
**温升对控制性能的影响链**：
```
温度升高 → RDS(on)增大 → 导通损耗增大 → 温度进一步升高（正反馈！）
温度升高 → 磁链减弱 → 反电动势降低 → 相同转速下需要更少弱磁
温度升高 → 绕组电阻增大 → 电气时间常数变化 → 电流环带宽变化
```
...
1. 传导干扰路径：
   PWM开关 → 电源纹波 → ADC基准偏移 → 采样误差

2. 辐射干扰路径：
   功率回路辐射 → 信号走线耦合 → 采样信号毛刺
