...
死区时间计算：
  ...
  STM32G4示例：
    Tdtg = Tclk / (CKD × 2)
    CKD = 1, Tclk = 170MHz → Tdtg = 5.88ns
    DTG = 0x40 → TD = (64+1) × 5.88ns = 382ns
...
示例：
  时钟频率 = 170MHz
  开关频率 = 10kHz (中心对齐)
  ARR = 170MHz / (2 × 10kHz) = 8500
  分辨率 = 1/8500 ≈ 0.012%

  开关频率 = 20kHz (中心对齐)
  ARR = 170MHz / (2 × 20kHz) = 4250
  分辨率 = 1/4250 ≈ 0.024%
...
步骤1：计算Vα, Vβ (逆Park变换输出)
步骤2：判断扇区
步骤3：计算T1, T2, T0
步骤4：计算CCR1, CCR2, CCR3
步骤5：写入PWM比较寄存器

扇区判断：
  if (Vbeta > 0) {
      if (Valpha > 0) {
          if (Vbeta > sqrt(3) * Valpha) sector = 2;
          else sector = 1;
      } else {
          if (Vbeta > -sqrt(3) * Valpha) sector = 3;
          else sector = 2;
      }
  }

CCR计算（以扇区1为例）：
  T1 = sqrt(3) * Ts / Vdc * Vbeta
  T2 = sqrt(3) * Ts / Vdc * (Valpha - Vbeta/sqrt(3))
  T0 = Ts - T1 - T2

  CCR1 = (T0/4 + T1/2 + T2/2) × ARR
  CCR2 = (T0/4 + T2/2) × ARR
  CCR3 = (T0/4) × ARR
```
...
时序分析：
  T0: PWM谷底(计数器=0)
  T1: ADC触发(延迟T0→T1 = 触发延迟)
  T2: 采样保持开始(T1→T2 = 触发到采样 = ~0)
  T3: 采样保持结束(T2→T3 = 采样时间)
  T4: 转换完成(T3→T4 = 转换时间)
  T5: 数据可用(T4→T5 = DMA传输)
...
中断延迟构成：
  ...
  示例(170MHz STM32G4)：
    最小延迟 = 24/170MHz ≈ 141ns
    典型延迟 = 34/170MHz ≈ 200ns
...
STM32G4时钟树(示例)：
  ...
  HSE(8MHz) → PLL → SYSCLK(170MHz)
                  │
                  ├→ AHB(170MHz) → DMA, Flash
                  ├→ APB1(170MHz) → TIM2-7, USART2-3, SPI2-3
                  └→ APB2(170MHz) → TIM1, ADC1-5, SPI1, USART1
...
```
方案1：MCU内部比较器
  ADC采样电流 → 与阈值比较 → 触发中断/关闭PWM

  优点：灵活(软件可调阈值)
  缺点：延迟大(ADC+比较+中断 ≈ 5~10μs)
...
```
独立看门狗(IWDG)：
  超时时间 = (RLR + 1) × Prescaler / LSI_FREQ

  示例：
    LSI = 32kHz, Prescaler = 32, RLR = 1000
    超时 = 1001 × 32 / 32000 = 1.0s
...
死区对输出电压的影响：
  ...
  理想输出电压：Vout = D × Vdc
  实际输出电压：Vout = D × Vdc - ΔV_dead

  死区电压损失：
    ΔV_dead = (Td / Tpwm) × Vdc × sign(I)

    当I > 0时：ΔV_dead > 0 (电压减小)
    当I < 0时：ΔV_dead < 0 (电压增大)
    当I = 0时：ΔV_dead = 0 (过零点不连续)

  示例：
    Td = 500ns, Tpwm = 50μs (20kHz), Vdc = 48V
    ΔV_dead = (0.5/50) × 48 = 0.48V
...
时序错误的影响：
  ...
  时序偏差分析：
    偏离中点Δt → 电流误差 ≈ di/dt × Δt
    示例：di/dt = 50A/ms, Δt = 2μs → 误差 = 0.1A
...
**🔗 算法关联**：中断优先级配置决定控制周期的确定性
```
优先级配置错误的影响：

  错误1：通信中断优先级高于控制中断
    现象：控制周期被通信中断抢占 → 周期抖动
    示例：通信中断耗时50μs → 控制周期从100μs变为150μs
    影响：电流环积分误差 → 电流偏移

  错误2：控制中断中调用阻塞函数
    现象：控制中断执行时间不确定
    示例：在控制中断中调用printf → 耗时100μs
    影响：控制周期严重抖动

  错误3：过流保护优先级不够高
    现象：过流时响应延迟
    示例：过流到关断延迟10μs → 电流可能已超限
    影响：器件损坏风险
...
PWM分辨率不足的影响：
  ...
  电压量化台阶 = Vdc / ARR
  示例：Vdc = 48V, ARR = 4250 → 11.3mV/LSB

  对电流的影响：
    ΔI = ΔV / (L × fsw)
    示例：L = 1mH, fsw = 20kHz → ΔI = 11.3mV / (1mH × 20kHz) = 0.57mA

  通常PWM分辨率足够，不是主要误差源

  但在低占空比时：
    D = 1/4250 = 0.024% → 最小非零占空比
    对应电压 = 48V × 0.024% = 11.3mV
...
DMA对CPU负载的影响：
  ...
  典型CPU负载：
    FOC算法：20~40μs (STM32G4 @ 170MHz)
    DMA节省：2~5μs
    总控制周期：25~45μs
    控制频率：10~20kHz → CPU负载25~90%
```
...
步骤2:分析尖峰位置 → 对应扇区切换点
步骤3:检查PWM死区 → Td = 500ns
步骤4:计算死区电压损失 → ΔV = 0.48V (9.6%误差)
```
...
步骤4:计算8μs内电流上升 → I = V/L × t = 48V/0.5mH × 8μs = 0.768A
```

**根本原因**：软件过流保护响应太慢，8μs内电流上升0.768A——虽然额外增量不大，但堵转时初始电流已达数十安培，叠加后仍可能超过MOSFET承受能力，需要硬件级快速保护

**解决方案**：
```
方案A:硬件比较器 + PWM刹车输入
  过流 → 比较器翻转 → PWM刹车 → 关断
  响应时间 < 1μs → 电流上升 < 0.096A，有效限制故障电流 ✅
...
参考答案：
1. ΔV = Td/Tpwm × Vdc = 0.5μs/50μs × 48V = 0.48V
2. ΔI = ΔV / (L × fsw) = 0.48V / (1mH × 10kHz) = 48mA (稳态)
   但在电流过零点，死区影响更大(电压跳变)
3. 补偿量 = Td/Tpwm × sign(I) = 0.01 × sign(I)
   在占空比中加减补偿量
```
...
2. 最坏情况：控制中断被过流保护抢占1μs → 51μs
3. 通信中断30μs > 控制周期50μs - 25μs(算法) = 25μs
   通信中断不能在控制周期空闲时完成！
   解决：通信使用DMA，减少中断时间到5μs
```
