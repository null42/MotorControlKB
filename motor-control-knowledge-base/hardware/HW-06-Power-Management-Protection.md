...
1级：限速提醒（电流限制 1.2×In）→ 软件限幅
2级：超速警告（过流预警 1.5×In）→ 记录计数
3级：刹车干预（过流保护 2.0×In）→ 关断PWM
4级：紧急制动（短路保护 3.0×In）→ 硬件立即关断
```
...
故障分类
    │
    ├─── 致命故障（短路、IGBT损坏）──→ 立即停机，禁止重启
    │
    ├─── 严重故障（过流、过压、过温）──→ 停机，延时后可自动恢复
    │
    └─── 轻微故障（预警、通信异常）──→ 降额运行，记录日志
...
```
系统需求：母线电压48V DC，额定功率2kW，额定电流50A，峰值电流100A
...
```
系统需求：母线电压540V DC（380V AC整流后），额定功率15kW，额定电流30A，峰值电流60A
