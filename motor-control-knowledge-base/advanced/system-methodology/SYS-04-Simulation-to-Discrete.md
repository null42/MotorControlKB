...
```
仿真怎么搭？          → PMSM模型 + 逆变器模型 + 控制器模型，步长1e-6~1e-7
连续域怎么设计？      → 电流环零极点对消，速度环对称最优
离散化有哪些方法？    → ZOH / FOH / Tustin / 预畸变Tustin / 零极点匹配
哪种方法最好？        → 控制器用Tustin，被控对象用ZOH
误差从哪来？          → 频率畸变 + 相位延迟 + 采样延迟 + 计算延迟
延迟有多大？          → 总延迟 = 0.5Ts(采样) + Ts(计算) = 1.5Ts
采样频率怎么选？      → fs >= 10 x fBW，电流环通常10~20kHz
代码怎么写？          → 增量式PI + 抗积分饱和 + 输出限幅
新算法怎么验证？      → 10步流程：文献→连续建模→连续仿真→整定→离散化→离散仿真→加延迟→C代码→HIL→微调
```
