...
$$f_{PWM} = \frac{f_{TIM}}{2 \times ARR}$$

**注意2倍关系**：中心对齐模式下，计数器从0到ARR再回到0才算一个完整周期，所以实际周期是 $2 \times ARR$ 个时钟周期。

### 5.2 边沿对齐模式频率

$$f_{PWM} = \frac{f_{TIM}}{ARR}$$
...
中心对齐模式：
f_PWM = 170,000,000 / (2 × 4250) = 20,000 Hz = 20 kHz
```

> **注意**：ARR的值是4250还是4249取决于是否减1。STM32的ARR寄存器从0计数到ARR值，所以周期为ARR+1个计数。但在实际工程中，CubeMX生成的代码通常直接写Period=4250，对应ARR=4250，实际周期为4251个计数。精确计算时需确认。

**修正公式**：

$$f_{PWM} = \frac{f_{TIM}}{2 \times (ARR + 1)}$$

但工程中通常忽略这个+1的微小差异（误差<0.024%）。
...
void Motor_PWM_Init(void)
{
    ...
    TIM1->ARR = 4250 - 1;                   // ARR=4249, f_PWM = 170M/(2*4250) ≈ 20kHz
