...
```
为什么需要标幺值？   → 统一数值范围，便于定点数表示和算法复用
基值怎么选？         → 电压/电流基值决定所有派生基值，必须满足一致性
标幺值下PI参数有意义吗？ → 有！Kp和Ki直接关联电机物理参数
定点数Q格式怎么选？   → 根据变量范围和精度要求，电机控制常用Q14/Q15
Q15乘法怎么不溢出？   → Q15×Q15=Q30，需要移位和饱和处理
什么时候必须用定点？   → 无FPU的MCU、极高实时性、代码空间受限
CORDIC有什么用？      → 无乘法器计算sin/cos，STM32G4硬件加速
```
