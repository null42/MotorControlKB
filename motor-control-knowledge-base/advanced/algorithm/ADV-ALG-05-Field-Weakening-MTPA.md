...
```
为什么需要弱磁？     → 反电动势随转速升高，电压饱和
电压约束长什么样？   → dq平面上是一个椭圆（随转速缩小）
电流约束长什么样？   → dq平面上是一个圆
MTPA是什么？         → 电流圆上转矩最大的点
弱磁区域怎么划分？   → 椭圆和圆的相对位置决定
弱磁电流怎么算？     → 电压反馈法/前馈计算法/查表法
MTPA和弱磁怎么协调？ → 电压约束优先，取更负的Id
深度弱磁怎么处理？   → 过调制+六步方波
弱磁稳定吗？         → 参数敏感，需仔细设计PI
```
...
```
                    ┌──────────────────────────────────┐
                    │                                  │
  V_ref ──→(+)──→[PI_fw]──→ Id_ref ──→[电流环]──→ Id ──→[电机]──→ V_s
            ↑ -                                                    │
            └──────────────────────────────────────────────────────┘
```
