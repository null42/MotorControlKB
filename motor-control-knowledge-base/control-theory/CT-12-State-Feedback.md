...
状态空间模型：ẋ = Ax + Bu
    ↓
验证能控性：rank[B AB ... Aⁿ⁻¹B] = n
    ↓
选择期望极点（基于性能指标：ζ, ωn, ts）
    ↓
计算状态反馈增益K（Ackermann公式 / 直接求解）
    ↓
实现：u = -Kx + Nr（r为参考输入，N为前馈增益）
```
...
$$
\mathbf{K} = \begin{bmatrix} 2\omega_c L - R & \omega_e L \\ -\omega_e L & 2\omega_c L - R \end{bmatrix}
$$
...
$$
u_d = -(2\omega_c L - R)i_d - \omega_e L \cdot i_q
$$
$$
u_q = \omega_e L \cdot i_d - (2\omega_c L - R)i_q
$$
...
$$
\mathbf{K} = \begin{bmatrix} 2\times2000\times0.0005-0.08 & 1000\times0.0005 \\ -1000\times0.0005 & 2\times2000\times0.0005-0.08 \end{bmatrix} = 
\begin{bmatrix} 1.92 & 0.5 \\ -0.5 & 1.92 \end{bmatrix}
$$

**控制律：**
- ud = -1.92·id - 0.5·iq + 1.92·id_ref（N的简化形式）
- uq = 0.5·id - 1.92·iq + 1.92·iq_ref
...
$$K = \begin{bmatrix} 2.92 & 0.5 & 0.02 \\ -0.5 & 2.92 & 0.08 \end{bmatrix}$$
...
| PMSM电流K | $k_{ii}=2\omega_c L-R$, $k_{ij}=\pm\omega_e L$ | 自动解耦 |
