...
```text
1. 注入 id = I_rated, iq = 0（d轴对齐）
   → 定子产生与转子永磁体对齐的磁场
   → 转子锁定在电角度 θ_e = 0 位置

2. 读取编码器原始角度 θ_enc_raw

3. 计算偏移：
   θ_offset = -θ_enc_raw / p  (因为对齐后 θ_e=0)
```
...
```text
验证流程：
1. 偏移校准后，id=5A, iq=0 → 确认编码器读出角度稳定
2. 施加 iq=5A, id=0 → 确认转矩输出无振荡
3. 速度闭环运行 → 确认电流波形正弦，无直流偏置
```
...
```text
参数误差 → PI参数偏差 → 电流响应恶化 → 转矩精度下降 → 速度/位置精度恶化
```
