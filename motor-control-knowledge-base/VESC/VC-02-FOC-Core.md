...
```
                            ...
                            └─────────────┬────────────────────────────┘
                                          ...
                                          ▼
  ┌───────────────────────────────────────────────────────────────────────┐
  │                     mcpwm_foc_adc_int_handler (ISR)                    │
  │                                                                        │
  │  Step 1: 读取 ADC 注入结果                                             │
  │          Ia = GET_CURRENT1() × FAC_CURRENT1                            │
  │          Ib = GET_CURRENT2() × FAC_CURRENT2                            │
  │          Ic = GET_CURRENT3() × FAC_CURRENT3 (3-shunt)                  │
  │          ─────────────────────────────                                 │
  │  Step 2: 获取电角度                                                    │
  │          编码器 / 霍尔 / 观测器 / HFI                                  │
  │          foc_observer_update() → phase                                 │
  │          foc_pll_run()         → filtered_phase, speed                 │
  │          ─────────────────────────────                                 │
  │  Step 3: Clarke 变换 (abc → αβ)                                       │
  │          i_alpha = i_a                                                 │
  │          i_beta = (i_a + 2×i_b) / √3                                  │
  │          ─────────────────────────────                                 │
  │  Step 4: Park 变换 (αβ → dq)                                          │
  │          id = cosθ × i_alpha + sinθ × i_beta                          │
  │          iq = cosθ × i_beta - sinθ × i_alpha                          │
  │          ─────────────────────────────                                 │
  │  Step 5: 电流 PI 控制 (control_current)                               │
  │          vd = vd + Kp×(id_set - id) + Ki×dt×(id_set - id)            │
  │          vq = vq + Kp×(iq_set - iq) + Ki×dt×(iq_set - iq)            │
  │          ─────────────────────────────                                 │
  │  Step 6: 弱磁控制 (foc_run_fw)                                        │
  │          注入 Id 电流以扩展速度范围                                    │
  │          ─────────────────────────────                                 │
  │  Step 7: 逆 Park 变换 (dq → αβ)                                       │
  │          v_alpha = cosθ × vd - sinθ × vq                              │
  │          v_beta  = sinθ × vd + cosθ × vq                              │
  │          ─────────────────────────────                                 │
  │  Step 8: SVPWM 调制 (foc_svm)                                         │
  │          六扇区空间矢量 → tA, tB, tC                                   │
  │          ─────────────────────────────                                 │
  │  Step 9: PWM 占空比更新                                               │
  │          TIMER_UPDATE_DUTY_M1(tA, tB, tC)                             │
  │                                                                        │
  └───────────────────────────────────────────────────────────────────────┘
```
