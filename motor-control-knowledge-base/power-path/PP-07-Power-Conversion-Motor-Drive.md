...
```
水库(AC电网) → 水处理(PFC) → 水塔(DC母线) 
    → 家庭用水(辅助电源) + 工厂用水(逆变器→电机)
    → 废水回收(制动能量处理)

母线电容 = 水塔的储水罐
  储水罐够大 → 用水高峰水压不跌
  但储水罐太大 → 成本高，且需要定期维护(电容老化)

辅助电源 = 家庭用水管道
  水塔 → 高压主管 → 减压阀(Buck) → 家庭水龙头(MCU)
  每个家庭(栅极驱动)需要独立的管道(隔离电源)
...
```
三相 AC      EMI     整流桥    PFC/      DC母线    三相      电机
380VAC ─→ Filter ─→ 6-Diode ─→ Boost ─→ Cdc ─→ Inverter ─→ PMSM
  │                    │         │         │         │
  │                    │         │    ┌────┤         │
  │                    │         │    │    │    制动斩波器
  │                    │         │    │    │    R_brake
  ...
  │              ┌────────────┼──────────────┤
  │              │            │              │
  │         15V(隔离)   15V(非隔离)    5V/3.3V
  │         三路上桥臂   下桥臂+其他    MCU+传感器
  │         栅极驱动     栅极驱动
```
...
```
Relay 并联预充电电阻 R_precharge
  上电 → R_precharge 限制电流 → 电容缓慢充电
  → 当 Vdc > 设定值（如 250V）→ Relay 闭合短路 R_precharge
```
...
母线电压 Vdc ───→ 电压前馈 ───→ SVPWM 调制比修正
母线电容 Cdc ───→ 母线电压纹波 ───→ 电流采样窗口选择
辅助电源    ───→ DSP 运行    ───→ FOC 中断及时性
栅极驱动电源 ───→ MOSFET 开关 ───→ 死区时间精度
制动斩波器  ───→ 母线电压管理 ───→ 过压保护策略
```
