...
**模态2：开关管关断（释能阶段）**
```
变压器次级 → 二极管 → Cout//Rload → 次级
磁芯能量通过次级释放

次级电压：Vs = Vout + VF（二极管导通压降）
初级反射电压：Vp = Vs × (Np/Ns) = (Vout+VF) × N
其中 N = Np/Ns（匝比）
```
...
```
Vin → 变压器初级 → MOSFET(ON) → GND
次级感应电压，二极管 D1 导通，向 L 和负载供电
VL = Vs - Vout = Vin/N - Vout
```

**模态2：开关管关断**
```
初级无电流，次级 D1 关断，D2（续流二极管）导通
L 通过 D2 向负载释放能量
VL = -Vout
```
...
310V DC 母线
    │
    └── Flyback 变压器
         ├── 15V/2A (隔离) → 上桥臂栅极驱动 U 相
         ├── 15V/2A (隔离) → 上桥臂栅极驱动 V 相
         ├── 15V/2A (隔离) → 上桥臂栅极驱动 W 相
         ├── 15V/0.5A (隔离) → 下桥臂栅极驱动（共用）
         └── 5V/1A (隔离) → MCU + 传感器
```
