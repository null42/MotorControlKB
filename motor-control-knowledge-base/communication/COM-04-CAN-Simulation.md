...
```
                    CAN 2.0A 标准数据帧（最大130位，含填充位前）
┌────┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬───┬────┬───┬───┬────┐
│SOF │ID10│ID9│ID8│ID7│ID6│ID5│ID4│ID3│ID2│ID1│ID0│RTR│IDE │r0 │DLC │    仲裁场 + 控制场
│ 1b │    11位标识符（高位先发）    │   │   │   │3b │
├────┴───┴───┴───┴───┴───┴───┴───┴───┴───┴───┼───┼───┼────┤
│                                              │   │   │    │
│  仲裁场 (12位)                               │控制场(6位)│
...
│  CRC场 (16位)                                              │
│  ┌───────────────────────┬─────┐                           │
│  │  CRC序列 (15位)        │ CD  │  CD = CRC界定符(隐性1位)  │
│  └───────────────────────┴─────┘                           │
├────────────────────────────────────────────────────────────┤
│  ACK场 (2位)                                               │
│  ┌─────┬─────┐                                            │
│  │ ACK │ AD  │  ACK=发送方隐性/接收方显性, AD=界定符(隐性)  │
│  │ 槽  │     │                                             │
│  └─────┴─────┘                                            │
├────────────────────────────────────────────────────────────┤
│  EOF (7位隐性) + IFS (3位隐性)                             │
...
```
                    CAN 2.0B 扩展数据帧
┌────┬────────────────┬───┬───┬───┬────────────────┬───┬───┬───┬────┐
│SOF │ Base ID[28:18] │SRR│IDE│ Ext ID[17:0]    │RTR│r1 │r0 │DLC │
│ 1b │ 11位           │1b │1b │ 18位            │1b │1b │1b │3b  │
│    │ (高位先发)      │隐性│隐性│                 │   │   │   │    │
└────┴────────────────┴───┴───┴────────────────┴───┴───┴───┴────┘
  │←────── 仲裁场 (32位) ──────────────────────────────→│←控制场→│
  │                                                     │ 6位   │
...
仲裁场详解:
  Base ID[28:18]: 29位ID的高11位，与标准帧ID位置相同
  SRR (Substitute Remote Request): 固定为隐性1，替代标准帧的RTR位
  IDE (Identifier Extension): 固定为隐性1，标识扩展帧
  Ext ID[17:0]: 29位ID的低18位
  RTR: 数据帧=显性0，远程帧=隐性1
```
...
```
                    CAN FD 数据帧（BRS=1）
┌────┬────────────┬───┬───┬───┬───┬───┬───┬────┐
│SOF │ ID + RTR   │IDE│FDF│r1 │BRS│ESI│DLC│    控制场（扩展）
│ 1b │ 11/29位    │   │ 1 │   │   │   │4b │
│    │ RTR固定隐性│   │   │   │   │   │   │
└────┴────────────┴───┴───┴───┴───┴───┴───┴────┘
  │←── 标称波特率 ──→│←─ 标称波特率 ─→│
...
```
模拟场景: 节点A的CAN_H线短路到GND

时间轴 →
┌─────────────────────────────────────────────────────────────────────┐
│ TEC   │ 0  → 8 → 16 → 24 → ... → 96 → 104 → 112 → 120 → 128    │
│       │     ↑    ↑    ↑         ↑     ↑      ↑       ↑      ↑     │
│ 状态  │ Active ──────────────────────→ 预警 ──────────→ Passive    │
│       │                              (TEC>96)        (TEC>127)     │
│       │                                                              │
│ 行为  │ 主动错误标志(6位显性)        被动错误标志(6位隐性)          │
│       │ 破坏其他节点帧               不影响其他节点                 │
│       │                                                              │
│ TEC   │ 128 → 136 → ... → 248 → 256                                │
│       │  ↑              ↑      ↑                                     │
│ 状态  │ Passive ─────────────→ Bus Off (TEC>255)                    │
│       │                                                              │
│ 行为  │ 完全脱离总线，不发送任何数据                                  │
│       │                                                              │
│ 恢复   │ Bus Off → 监控128×11位隐性位 → Error Passive → Error Active│
│       │ (需软件请求恢复)                                              │
└─────────────────────────────────────────────────────────────────────┘

每个发送失败帧TEC增加8，成功帧TEC减少1
从TEC=0到Bus Off(TEC=256)需要连续32次发送失败
