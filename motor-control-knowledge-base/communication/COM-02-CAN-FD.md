...
CAN FD数据帧结构：
┌─────┬──────────────────┬──────────────────────────┬──────────────┬──────────┬─────┬─────┬──────┐
│ SOF │ 仲裁场           │ 控制场（扩展）            │ 数据场       │ CRC场    │ ACK │ EOF │ IFS  │
│ 1bit│ 11/29bit ID+     │ IDE+FDF+r1+BRS+ESI+DLC   │ 0~64字节     │ 变长     │2bit │7bit │3bit  │
│     │ RTR(固定隐性)    │                           │              │          │     │     │      │
└─────┴──────────────────┴──────────────────────────┴──────────────┴──────────┴─────┴─────┴──────┘
```
...
```
BRS=1时的位速率切换时序：
仲裁段（标称波特率）                    数据段（数据波特率）           仲裁段
┌──────────────────────────┐ BRS ┌──────────────────────────────────┐ CRC ┌─────┐
│ SOF → 仲裁场 → 控制场    │  1  │ 数据场 → CRC场                   │ 界定│ ACK │
│ (如500 kbit/s)           │     │ (如5 Mbit/s)                    │ 符  │     │
└──────────────────────────┘     └──────────────────────────────────┘     └─────┘
                              ↑                                         ↑
                         速率切换点                                速率切换回
```
...
```
CAN 2.0 (8字节):
  需要拆帧：位置(4B) + 速度(2B) + 电流(2B) = 8字节 → 仅够基本反馈
  状态字、故障码、温度等需要额外帧

CAN FD (64字节):
  单帧传输：位置(4B) + 速度(2B) + 电流(2B) + 电压(2B) + 温度(2B)
           + 状态字(2B) + 故障码(2B) + 保留(48B) = 64字节
  所有关键信息一帧搞定
```
...
CAN FD CRC场结构：
┌────────────────────────┬────────────────────────┬──────────────┐
│ Stuff Count (4位灰度码) │ Parity (1位)           │ CRC序列      │
│ 记录填充位的个数        │ Stuff Count的偶校验    │ 17位或21位   │
│ 范围0~15               │                        │              │
└────────────────────────┴────────────────────────┴──────────────┘
```
