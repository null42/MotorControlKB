...
bool Controller::update() {
    ...
    if (config_.control_mode < CONTROL_MODE_VELOCITY_CONTROL) {
        ...
        vel_integrator_torque_ = 0.0f;
    } else if (config_.enable_vel_limit) {
        if (limited) {
            // TODO: limit integral to not contribute to windup
        } else {
    ...
    // Spinout detection
    // TODO: move to motor.cpp
    mechanical_power_ += current_meas_period * config_.mechanical_power_bandwidth * ((*vel_estimate) * torque - mechanical_power_);
    electrical_power_ += current_meas_period * config_.electrical_power_bandwidth * (axis_->motor_.electrical_power() - electrical_power_);
    if (config_.spinout_mechanical_power_threshold > 0.0f && mechanical_power_ > config_.spinout_mechanical_power_threshold) {
        set_error(ERROR_SPINOUT_DETECTED);
        return false;
    }
    if (config_.spinout_electrical_power_threshold > 0.0f && electrical_power_ > config_.spinout_electrical_power_threshold) {
        set_error(ERROR_SPINOUT_DETECTED);
        return false;
    }
