...
void hard_init(void)
{
...
#ifdef  HALL_SENSOR_SELECT
  TIM_TimeBaseInitTypeDef TIM_HALLTimeBaseInitStructure;
  TIM_ICInitTypeDef       TIM_HALLICInitStructure;
#endif
#ifdef  ENCODER_SENSOR_SELECT
  TIM_TimeBaseInitTypeDef TIM_ENCODERTimeBaseInitStructure;
  TIM_ICInitTypeDef       TIM_ENCODERICInitStructure;
#endif
...
#ifdef  HALL_SENSOR_SELECT            //通过条件编译配置霍尔传感器  
...
#ifdef  ENCODER_SENSOR_SELECT            //通过条件编译配置编码器传感器 
  GPIO_StructInit(&GPIO_InitStructure);
  ...
  GPIO_StructInit(&GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin = ENCODER_Z_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(ENCODER_Z_GPIO_PORT, &GPIO_InitStructure);
  
  GPIO_PinAFConfig(ENCODER_A_GPIO_PORT,ENCODER_A_SOURCE,ENCODER_A_AF);
  GPIO_PinAFConfig(ENCODER_B_GPIO_PORT,ENCODER_B_SOURCE,ENCODER_B_AF);
  GPIO_PinAFConfig(ENCODER_Z_GPIO_PORT,ENCODER_Z_SOURCE,ENCODER_Z_AF);
  
  
  TIM_DeInit(ENCODER_TIM);
  TIM_TimeBaseStructInit(&TIM_ENCODERTimeBaseInitStructure); 
  TIM_ENCODERTimeBaseInitStructure.TIM_Period = 4000;
  TIM_ENCODERTimeBaseInitStructure.TIM_ClockDivision = TIM_CKD_DIV1;
  TIM_TimeBaseInit(ENCODER_TIM,&TIM_ENCODERTimeBaseInitStructure);
  
  
  TIM_EncoderInterfaceConfig(ENCODER_TIM, TIM_EncoderMode_TI12, TIM_ICPolarity_Rising, TIM_ICPolarity_Rising);
  TIM_ICStructInit(&TIM_ENCODERICInitStructure);
  TIM_ENCODERICInitStructure.TIM_ICFilter = 0x00;
  TIM_ICInit(ENCODER_TIM, &TIM_ENCODERICInitStructure);
  
  TIM_ClearFlag(ENCODER_TIM, TIM_FLAG_Update);
  TIM_ITConfig(ENCODER_TIM, TIM_IT_Update, DISABLE);
  
  SYSCFG_EXTILineConfig(ENCODER_Z_EXTI_GPIO_PORT, ENCODER_Z_EXTI_SOURCE);

  
  EXTI_InitStructure.EXTI_Line = ENCODER_Z_EXTI_LINE;
