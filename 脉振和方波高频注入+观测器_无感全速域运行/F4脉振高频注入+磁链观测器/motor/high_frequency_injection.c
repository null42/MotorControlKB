...
{
    //滤波器参数初始化
    float WE_IIR_LPF_Coeff[8] = {1.0f, 1.0f, 0.0f, 1.0f, -0.969067417193793301f, 0.0f, 0.0154662914031034, 1.0f};
    ...
    float Q_IIR_BPF_Coeff[8] = {1.0f, 0.0f, -1.0f, 1.0f, -1.867249996121f, 0.96299405095f, 0.018502974524892f, 1.0f};
...
void HFI_Process(CURRENT_DQ_DEF current_dq_temp, CURRENT_DQ_DEF* current_dq_lpf, HFI_DEF* hfi_temp)
{
    ...
    float hfi_pid_error;
    float hfi_pid_out;
    if(hfi_cnt >= HFI_CNT_FLAG)  //HFI_CNT_FLAG = 电流环频率/注入频率 = 10K/500 = 20
    {
        hfi_cnt = 0;
    }
    temp1 = hfi_cnt;
    temp2 = HFI_CNT_FLAG;
    hfi_cos = arm_cos_f32((temp1 / temp2) * 2.0f * PI); //temp1从0到temp2为一个周期
    hfi_sin = arm_sin_f32((temp1 / temp2) * 2.0f * PI);
    if(HFI_inj_volt_amp_ok == 0)
    {
        if(HFI_inj_volt_amp_temp < HFI_inj_volt_amp) //将注入电压幅值从0往上加，直到到达设定值
        {
            HFI_inj_volt_amp_temp += 0.001f;
        }
        else
        {
            if(HFI_inj_volt_amp_ok_cnt > 1000) //到达设定值 1/10k*1000=0.1s 后，设置极性辨识标志
            {
    ...
    temp1 = q_current_bpf * hfi_sin;    //q轴高频电流乘以sin，进行幅值调制，低通滤波后得到 k*角度误差
    IIR_Butterworth(temp1, &hfi_pid_error, &ERR_IIR_LPF_Par);

    hfi_pid_out = HFI_PID.P_Gain * hfi_pid_error + HFI_PID.I_Sum;


    hfi_temp->hfi_theta = hfi_integrator;
    hfi_temp->hfi_we = hfi_pid_out;
    ...
    HFI_PID.I_Sum += (HFI_PID.I_Gain * hfi_pid_error) * FOC_PERIOD;
}
...
void d_direction_process(float in_temp, uint8_t* out_temp)
{
    if(HFI_inj_volt_amp_ok == 1)
    {
        if(d_direction_ok == 0)
        {
            if(id_index < 20) //对应一个正弦注入电压周期
            {
                if(in_temp > id_max) //得到一个周期幅值的包络
                {
                    id_max = in_temp;
                }
                if(in_temp < id_min)
            ...
            else
            {
                ...
                id_max = 0.0f;  //记录的id_max和id_min的数据只用一次，用完重新记录
                id_min = 0.0f;
                id_index = 0;
            }
        }
