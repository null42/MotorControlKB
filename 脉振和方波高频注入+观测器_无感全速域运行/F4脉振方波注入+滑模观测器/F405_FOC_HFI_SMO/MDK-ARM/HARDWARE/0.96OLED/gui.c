 ...
 * @retvalue   :None
******************************************************************************/   	  
void GUI_ShowString(u8 x,u8 y,char *chr,u8 Char_Size,u8 mode)
{
...
void GUI_ShowFont12(u8 x,u8 y,u8 *s,u8 mode)
{
	u8 i,j,k,tmp;
	u16 num;
	num = sizeof(cfont12)/sizeof(typFNT_GB12);
  for(i=0;i<num;i++)
	{
		if((cfont12[i].Index[0]==*s)&&(cfont12[i].Index[1]==*(s+1)))
		{
			for(j=0;j<24;j++)
			{
				if(mode)
...
void GUI_ShowNum_F(u8 x,u8 y,float num,u8 len,u8 len_f,u8 Size,u8 mode)				//显示带小数数字
{
	...
	num_temp=(u32)num;
	GUI_ShowNum(x+Size,y,num_temp,len,Size,mode);
	GUI_ShowChar(x+(len+0.5)*Size,y,'.',Size,mode);
	for(i=0;i<len_f;i++)
	{
		f_temp=(num-num_temp)*pow(10,i+1);
		num-=f_temp*pow(10,-(i+1));
		GUI_ShowNum(x+(len+1.5+i)*Size,y,f_temp,1,Size,mode);
	}
}

void GUI_ShowNum_FU(u8 x,u8 y,float num,u8 len,u8 len_f,u8 Size,u8 mode)
{
	u32 num_temp = 0;
	u8 i;
	u8 f_temp = 0;
	...
	num_temp=(u32)num;
	GUI_ShowNum(x,y,num_temp,len,Size,mode);
	GUI_ShowChar(x+(len-1)*Size-4,y,'.',Size,mode);
	for(i=0;i<len_f;i++)
	{
		f_temp=(num-num_temp)*pow(10,i+1);
		num-=f_temp*pow(10,-(i+1));
		GUI_ShowNum(x+(len+i-1)*Size+4,y,f_temp,1,Size,mode);
	}
}

void GUI_ShowNum_Int(u8 x,u8 y,s16 num,u8 len,u8 Size,u8 mode)
{
//	u16 temp_int;
	if(num<0)
	{
		...
		GUI_ShowChar(x,y,' ',Size,mode);
//	temp_int=(u16)num;
	GUI_ShowNum(x+Size,y,num,len,Size,mode);
}
