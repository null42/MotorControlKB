...
void Fault_Detection(void)
{
    ...
    if(Init_Over)									//初始化结束后进行电机状态异常检测
    {
        ...
        else if(ADC_Sample_F_Para.VBUS < Protect_Low_Voltage_V)
        {
            Fault_Code = 1;						//电压过低
            M_STOP();									//停机
            Motor.M_State = M_ERR;		//电机状态报错
        ...
        {
            if(my_abs(PARK_PCurr.Qs) >= (pi_spd.Umax - 0.3f))
                Stall_time ++;
        }
        else
            Stall_time = 0;
    ...
    //转速偏差过大检测
    /*
    if(Motor.M_State != STOP)
    {
    if(my_abs(pi_spd.Fbk) > 1000*Motor.P)					//1000RPM后，开启速度偏差检测
    {
...
void UVW_Axis_dq(void)
{
    ...
    if(Motor.M_State == M_HFI)						//基于HFI的无感控制下电角度获取
    ...
    else if(Motor.M_State == HS_C)				//低速HFI，高速SMO控制模式下，电角度与电角速度的获取
    {
        ...
        if(((Sensorless.Observer == 3) && (Sensorless.Observer_last == 1)) || (Up_Time != 0))			//当速度高于滞回上限值，观测器从HFI切换到SMO
        {
            ...
            else if(Up_Time == 400)	//切换完成时间，注意并不是400次检测到低于10°，而是在Up_Time等于2时低于即可，后续Up_Time会一直递增到400
            {
                Up_Time = 0;
            }
        }
...
void Speed_Closeloop(void)
{
    ...
    if(Speed_Cal_time == PWMZD_Count)			//1ms计算一次转速环
    {
        ...
        case IF_Control:
        {
            switch(QT_IF_Mode)
            {
                ...
                case 3:																	//IF启动
                {
                    ...
                    if(SMO_flag)													//开启SMO
                    {
                        Angle_SMO_Para.Ialpha = CLARKE_ICurr.Alpha; //反馈的值
                        Angle_SMO_Para.Ibeta = CLARKE_ICurr.Beta;
                        Angle_SMO_Para.Ualpha = SVPWM_dq.Ualpha; //上一次的SVPWM给定值
...
{
    vofa_data[0] = Motor.speed_M_rpm;										//电角度
    ...
    vofa_data[9] = Sensorless.Speed_Max;
...
void J_Scope_Write(void)
{
    ...
    JScope_data[3] = (s16)(Sensorless.Speed_Fbk);
