...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart->Instance == USART1)//如果是串口1
	{
		...
		*/
		...
		USART_RX_BUF[RxLine -1] = aRxBuffer[0];						//把每次接收到的数据保存到缓存数组
		if(aRxBuffer[0] == 0xfa)													//接收结束标志位，这个数据可以自定义，根据实际需求
		{
			USART_RX_STA = 1;																//接收完成标志位置一
			/*
			printf("RXLen=%d\r\n",RxLine);
      for(int i=0;i<RxLine;i++)
         printf("UART DataBuff[%d] = %x\r\n",i,USART_RX_BUF[i]);
			if(USART_RX_BUF[5] == 0xfb)
				VOFA_USART_PI_Adjust(USART_RX_BUF, 1);				//数据解析和参数赋值函数
			memset(USART_RX_BUF, 0, 8);  									//清空缓存数组
			RxLine=0;  																			//清空接收长度
			*/
		}
		aRxBuffer[0] = 0;																	//清空串口接收缓存
	}
}
/* USER CODE END 1 */
